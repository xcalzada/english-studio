...
const App = () => {
  ...
  useEffect(() => { setTheoryState({}); setVocabTestState({}); setReadingState({}); setWritingState(""); }, [selectedUnitId]);
  useEffect(() => { window.speechSynthesis.getVoices(); }, []);

  const renderNavHeader = (title) => (
    <div className="flex items-center space-x-4 mb-6">
      <button onClick={() => setView('unit-menu')} className="p-4 bg-white border-2 border-slate-100 hover:border-blue-400 rounded-2xl shadow-sm text-slate-600 transition-all"><ArrowLeft size={28} /></button>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] font-sans text-slate-900 pb-20">
      <nav className="bg-white/80 backdrop-blur-md border-b-2 border-slate-100 px-8 py-6 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="hidden md:flex bg-white px-5 py-2.5 rounded-2xl border-2 border-slate-100 shadow-sm items-center font-black text-xs text-slate-500 uppercase italic"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse mr-3"></div>{currentUnit ? `Unit: ${currentUnit.title}` : 'Select Lesson'}</div>
      </nav>
      ...
      <main className="container mx-auto px-4 mt-8">
        {view === 'topic-selector' && (
          <div className="flex flex-col items-center justify-center min-h-[500px] p-4 max-w-4xl mx-auto animate-in fade-in duration-700">
            <div className="text-center mb-12"><h1 className="text-6xl font-black text-slate-800 tracking-tighter uppercase italic italic">English <span className="text-blue-600">Studio</span></h1><p className="text-slate-500 mt-2 font-medium text-lg uppercase tracking-widest italic">Selecciona una lección para empezar</p></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
              {Object.values(UNITS_DATA).map((unit) => (
                <button key={unit.id} onClick={() => { setSelectedUnitId(unit.id); setView('unit-menu'); }} className="bg-white border-2 border-slate-100 p-8 rounded-[40px] text-left transition-all relative overflow-hidden group shadow-sm hover:border-blue-500 hover:shadow-2xl hover:translate-y-[-8px]">
                  <div className="flex justify-between items-start mb-6"><div className="p-5 bg-slate-50 rounded-2xl group-hover:bg-blue-50 transition-all text-blue-600"><Home size={32}/></div><ChevronRight className="text-slate-300 group-hover:text-blue-500 transition-colors" /></div>
              ...
              <div className="bg-slate-50 border-2 border-dashed border-slate-200 p-8 rounded-[40px] flex items-center justify-center text-slate-300 font-black uppercase italic tracking-widest">Lección 2 (Próximamente)</div>
            </div>
          </div>
        )}
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-t border-slate-50 py-4 text-center text-slate-400 text-[10px] font-black uppercase tracking-[0.4em] z-40">English Studio • UK Accent Edition</footer>
    </div>
  );
};

const UnitModuleButton = ({ icon, title, color, onClick }) => (
  <button onClick={onClick} className="flex flex-col p-6 bg-white border-b-4 border-slate-200 rounded-[32px] hover:translate-y-[-5px] hover:border-blue-400 hover:shadow-xl transition-all text-left group">
