...
export function useSpeech(fullText = '') {
  ...
  const pausedRef = useRef(false);  // FIX: track paused state in ref for use inside interval
  ...
  const play = useCallback((offset = 0, spd = speed) => {
    ...
    if (utt) {
      ...
      utt.onstart    = () => {
        if (mounted.current) { setPlaying(true); setPaused(false); pausedRef.current = false; }
        startSim(remaining.length, spd, fullText.length, offset);
        // FIX: only resume if user hasn't manually paused — prevents auto-resume bug
        chromeFixRef.current = setInterval(() => {
          if (speech.speaking && !pausedRef.current) { speech.pause(); speech.resume(); }
        }, 12000);
      };
    }
  ...
  const handlePause = useCallback(() => {
    ...
    pausedRef.current = true;  // FIX: mark as intentionally paused
